...
#|
This program operates on a fully-prefixed equation to output a constraint network list
--Example--
input: (= (+ (a) (b)) (ans))
output: ((adder l1 l2 ans) (constant a l1) (constant b l2))

ConstraintSystemBase provides the base functions used here
|#
...
; len-guard ex: if the size of the list is 1, and len-guard is 1,
; 	this will safely access the 1st element (index 0)
...
; return unique connector names in sequential order
; return unused name or "ans" if "ans" is found in expr
(define next-list-name
  (let ([count 0])
  	(lambda (expr)
  		(if (or (eq? (safe-list-ref expr 1) 'ans)
  				(null? (safe-list-ref expr 1)))
  			; returning "ans" will do nothing for nil cases
  			'ans
  			(begin (set! count (+ count 1))
  		   	   	   (string->symbol (string-append "l"
  		   	   	   						(number->string count))))))))

; hash table mapping symbols to their corresponding constraint type name
...
; uses processed subexpressions to build constraint list
...
; helper function for get-constraint-list (top down recursive processing)
; constrains right-hand side to left-hand side of eqtn by sharing the top layer (connector) name
(define (get-constraint-list-equaler expr top-layer-name)
  (let ([lhs (cadr expr)]	; "left hand side"
  		[rhs (caddr expr)])	; "righth and side"
		; check if 'ans is by itself on the right or left
  	  (if (or (eq? 'ans (car lhs))
  	  		  (eq? 'ans (car rhs)))
  	  	  (append
  	  		(get-constraint-list lhs 'ans)
  	  		(get-constraint-list rhs 'ans))
  	  	  ; else: use given top-layer-name to constraint lhs and rhs
  	  	  (append
...
; unary op: 				(op sub1)
; constant:					(op)
; future: improve this by taking a data-directed approach (table with lambda functions)
...
; call recursive helper to process raw prefix equation
